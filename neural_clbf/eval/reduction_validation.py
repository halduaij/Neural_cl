...
def rollout_rom(
    ...
    if dt is None:
        dt = sys.dt                      # exactly the integrator step of the FOM
...
def validate_reducer(
    ...
    Args:
        ...
        reducer: The dimension reducer to validate
        n_rollouts: Number of test trajectories
        horizon: Simulation time horizon in seconds
        dt: Integration timestep (defaults to sys.dt)
        device: Device to run validation on
        seed: Random seed for reproducibility
        input_mode: Control input mode ("zero", "random", or "nominal")
    ...
    # Define controller based on input_mode
    if input_mode == "zero":
        controller = lambda x: torch.zeros(x.shape[0], sys.n_controls, device=device)
    elif input_mode == "random":
        controller = lambda x: 0.1 * torch.randn(x.shape[0], sys.n_controls, device=device)
    ...
    # Simulate ROM
    rom_traj = rollout_rom(reducer, sys, x0, T, controller=controller, dt=dt)
    ...
    relative_error = mean_error / (full_norm + 1e-8)
