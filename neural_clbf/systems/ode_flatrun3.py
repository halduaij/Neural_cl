...
def train_mixed_clbf(
    ...
    for ep in range(epochs):
        ...
        # QP descent
        u_qp, r_qp = ctrl.solve_CLF_QP(X, requires_grad=False)
        r_mean = r_qp.mean()
...
# Make sure the QP “sees” the correct plant at the current time
            ...
            u, r = ctrl.solve_CLF_QP(x, requires_grad=False)
...
def simulate_flat_run(
    ...
    use_monotone: bool = True,
    droop_cfg: DroopConfig = DroopConfig(),
    clbf: CLFController = None,        # pass when ctrl=="clbf"
):
    ...
    for k in range(N):
        ...
        if args.ctrl == "monotone":
            ...
            # keep V_now/th_now consistent for plots/audits
...
def main():
    ...
    ap.add_argument("--ctrl", type=str, default="clbf", choices=["const","monotone","clbf"])
    ...
    clbf = None
    if args.ctrl == "clbf":
        # Create a temporary CPU version of the system for CLFController
        class CPULimitsWrapper:
            def __init__(self, sys):
                self.sys = sys
                
            def __getattr__(self, name):
                if name == 'control_limits':
                    upper, lower = self.sys.control_limits
                    return upper.cpu(), lower.cpu()
                elif name == 'state_limits':
                    upper, lower = self.sys.state_limits
                    return upper.cpu(), lower.cpu()
                else:
                    return getattr(self.sys, name)
        
        wrapped_sys = CPULimitsWrapper(sys)
        
        clbf = CLFController(
            dynamics_model=wrapped_sys,
            scenarios=[sys.nominal_params],
            experiment_suite=ExperimentSuite([]),
            clf_lambda=1.0,
            clf_relaxation_penalty=100.0,
    ...
    if args.train_mode == "mixed":
        train_mixed_clbf(
            ...
            batch_roll=1,   # e.g., 64 if batch=512
    ...
    if args.alter_amount not in (0.0, 1.0):
        ...
        for rb in reduced_buses:
            events.append(AlterEvent(bus_idx=int(rb), t=float(args.alter_t),
                                     amount=float(args.alter_amount), alter_tau=float(args.alter_tau)))
    ...
    simulate_flat_run(
        ...
        alter_events=events,
        ctrl=args.ctrl,           # NEW
        clbf=clbf,                # NEW (None unless ctrl=="clbf")
        droop_cfg=cfg
    )
