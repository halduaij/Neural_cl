...
# ---- LIMITERS (bounds derived from equilibrium; toggles OFF by default) ----
        ...
        self.enable_pv_first_order = False  # keep integrator style unless you turned on the new _f path
...
# In IEEE39ControlAffineDAE (call once after constructing goal_point)
    def _repair_equilibrium(self):
        ...
        self._last_V = V.detach().clone().to(torch.float64)
        self._last_theta = theta.detach().clone().to(torch.float64)
...
# ---------------- core dynamics f(x) ----------------
    def _f(self, x: torch.Tensor, params: dict) -> torch.Tensor:
        ...
        def _softbox(v, lo, hi, beta):
            return lo + (hi - lo) * (torch.tanh(beta * ((v - lo) / (hi - lo + 1e-12) - 0.5)) + 1.0) / 2.0

        def _maybe_box(v, lo, hi, enable: bool, beta):
            return _softbox(v, lo, hi, beta) if enable else v
...
# ---------------- AVR / Exciter ----------------
    ...
    class _KCLNewtonImplicit(torch.autograd.Function):
        """
        Forward: Newton solve of KCL(z; x)=0 for z=[theta_rel; V] with a square Jacobian.
        Backward: implicit gradient via (∂F/∂z)^T v = \bar z,  then grads = -(∂F/∂x)^T v.
        """

        @staticmethod
        def forward(ctx, sys, delta, Eqp, Ppv, Qpv):
            n = sys.n_gen
            device = Eqp.device

            # Warm start
            V0  = getattr(sys, "_last_V",  sys.Vset).to(device)
            th0 = getattr(sys, "_last_theta",
                        torch.zeros(n, device=device, dtype=V0.dtype))

            z = torch.cat([th0[1:], V0], dim=0).to(torch.float64).clone()  # (2n-1,)

            # Cast inputs once (forward uses no graph)
            delta64, Eqp64, Ppv64, Qpv64 = [t.to(torch.float64) for t in (delta, Eqp, Ppv, Qpv)]

            # Drop one KCL row → square system
            drop_kind, drop_bus = getattr(sys, "kcl_row_drop", ("imag", 0))
            drop_idx = (drop_bus if drop_kind == "real" else n + drop_bus)

            def F_full(zz):
                return sys._kcl_residual(zz, delta64, Eqp64, Ppv64, Qpv64)   # (2n,)

            mask = torch.ones(2*n, dtype=torch.bool, device=z.device); mask[drop_idx] = False
            def F_masked(zz):  # (2n-1,)
                return F_full(zz)[mask]

            itmax       = int(getattr(sys, "newton_itmax", 25))
            tol         = float(getattr(sys, "newton_tol", 1e-8))
            backtracks  = int(getattr(sys, "newton_backtracks", 2))

            for _ in range(itmax):
                Fm = F_masked(z)  # (2n-1,)
                # Square-J solve (no autograd tape in forward)
                J = torch.autograd.functional.jacobian(F_masked, z, create_graph=False, vectorize=True)
                try:
                    dz = torch.linalg.solve(J, -Fm)
                except RuntimeError:
                    dz = torch.linalg.lstsq(J, -Fm).solution

                # Short Armijo backtrack
                F0 = torch.norm(Fm)
                alpha = 1.0
                accepted = False
                for _ in range(backtracks):
                    z_try = z + alpha*dz
                    z_try = torch.cat([z_try[:n-1], torch.clamp(z_try[n-1:], 0.35, 1.60)], dim=0)
                    if torch.norm(F_masked(z_try)) <= (1.0 - 1e-3*alpha) * F0:
                        z, accepted = z_try, True
                        break
                    alpha *= 0.5
                if not accepted:
                    z = z + dz

                if torch.norm(F_masked(z)) < tol:
                    break
            ...
            # Cache warm starts (float64) and return float32 to match the rest of the model
            sys._last_V     = V.detach().clone()
            sys._last_theta = theta.detach().clone()
            V32, th32 = V.to(torch.float32), theta.to(torch.float32)

            # Save for backward (no graph)
            ctx.sys = sys
            ctx.n = n
            ctx.drop_idx = int(drop_idx)
            ctx.z_star  = z.detach().clone()        # float64
            ctx.delta64 = delta64.detach().clone()
            ctx.Eqp64   = Eqp64.detach().clone()
            ctx.Ppv64   = Ppv64.detach().clone()
            ctx.Qpv64   = Qpv64.detach().clone()
            return V32, th32

        @staticmethod
        def backward(ctx, grad_V, grad_theta):
            sys, n, drop_idx = ctx.sys, ctx.n, ctx.drop_idx

            # Map output grads to z=[theta_rel; V]
            grad_z = torch.cat([grad_theta.to(torch.float64)[1:], grad_V.to(torch.float64)], dim=0)

            # Rebuild variables with grad for a local VJP
            z      = ctx.z_star.clone().requires_grad_(True)
            delta  = ctx.delta64.clone().requires_grad_(True)
            Eqp    = ctx.Eqp64.clone().requires_grad_(True)
            Ppv    = ctx.Ppv64.clone().requires_grad_(True)
            Qpv    = ctx.Qpv64.clone().requires_grad_(True)

            def F_full(zz):
                return sys._kcl_residual(zz, delta, Eqp, Ppv, Qpv)

            mask = torch.ones(2*n, dtype=torch.bool, device=z.device); mask[drop_idx] = False
            def F_masked(zz):
                return F_full(zz)[mask]

            # Build J_z and solve J_z^T v = grad_z
            J = torch.autograd.functional.jacobian(
                lambda zz: F_masked(zz), z, create_graph=False, vectorize=True
            )  # (2n-1, 2n-1)
            try:
                v = torch.linalg.solve(J.T, grad_z)
            except RuntimeError:
                v = torch.linalg.lstsq(J.T, grad_z).solution

            # -(∂F/∂x)^T v for each input x
            g_delta = torch.autograd.grad(F_masked(z), delta, grad_outputs=v, retain_graph=True,  allow_unused=True)[0]
            g_Eqp   = torch.autograd.grad(F_masked(z), Eqp,   grad_outputs=v, retain_graph=True,  allow_unused=True)[0]
            g_Ppv   = torch.autograd.grad(F_masked(z), Ppv,   grad_outputs=v, retain_graph=True,  allow_unused=True)[0]
            g_Qpv   = torch.autograd.grad(F_masked(z), Qpv,   grad_outputs=v, retain_graph=False, allow_unused=True)[0]

            # Replace Nones with zeros and cast back to model dtype
            def nz(x, like): return (torch.zeros_like(like) if x is None else x).neg().to(torch.float32)
            g_delta = nz(g_delta, ctx.delta64)
            g_Eqp   = nz(g_Eqp,   ctx.Eqp64)
            g_Ppv   = nz(g_Ppv,   ctx.Ppv64)
            g_Qpv   = nz(g_Qpv,   ctx.Qpv64)

            # No grad for 'sys'
            return (None, g_delta, g_Eqp, g_Ppv, g_Qpv)
    def _kcl_residual(
        ...
        n       = self.n_gen
        device  = z.device
        rdtype  = z.dtype                                 # float32 or float64
    ...
    def _solve_kcl_newton(
        ...
        z = torch.cat([th0[1:], V0], dim=0).to(torch.float64).clone().requires_grad_(True)
        delta64, Eqp64, Ppv64, Qpv64 = [t.to(torch.float64) for t in (delta, Eqp, Ppv, Qpv)]
        ...
        for it in range(itmax):
            ...
            def _res(zz):
                return self._kcl_residual(zz, delta64, Eqp64, Ppv64, Qpv64)

            ZB = z.unsqueeze(0).expand(3, -1).clone().requires_grad_(True)
            try:
                vmap(_res)(ZB)  # will throw exactly at the first non‑vmap‑compatible op
            except Exception as e:
                print("[VMAP-PROBE] vmap failed in _kcl_residual path:", e)
                raise
            J_full = jacobian(
                lambda zz: self._kcl_residual(zz, delta64, Eqp64, Ppv64, Qpv64),
                z, create_graph=True, vectorize=True  
            )                                                                 # (2n, 2n-1)

            JT = J_full.transpose(0, 1)
            H  = JT @ J_full
            g  = JT @ F_full
            def _cond_est(M):
        ...
        theta_rel = z[:self.n_gen-1]                 # keep float64 here
        V         = torch.clamp(z[self.n_gen-1:], 0.35, 1.60)  # float64
    ...
    #    print(f"[PF-COND] cond(J_full) ~ {_cond_est(J_full):.3e}")
     #   print(f"[PF-COND] cond(JT J)   ~ {_cond_est(JT @ J_full):.3e}")
        ...
        # cache warm starts in float64 if you like, but return 32 only at the very end
        self._last_V = V.detach().to(torch.float64)
        self._last_theta = theta.detach().to(torch.float64)
        return V32, theta32
