...
def monotone_linear_u(sys: IEEE39ControlAffineDAE, x: torch.Tensor, cfg: DroopConfig):
    ...
    u_cmd =0* torch.cat([uP, uQ], dim=-1).unsqueeze(0)  # shape (1, 2n)
...
if hasattr(sys, "_repair_equilibrium"):
    ...
    sys.enable_vr_soft_clip = True       # AVR command box
    sys.enable_efd_box      = True       # Efd ceiling/floor
    sys.enable_pref_box     = True       # droop output envelope
    sys.enable_pvalve_box   = True       # valve travel
    sys.enable_pvalve_rate  = True       # valve slew
    sys.enable_pm_box       = True       # mechanical power box
