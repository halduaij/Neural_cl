...
class _KCLNewtonImplicit(torch.autograd.Function):
    ...
    def forward(ctx, sys, delta, Eqp, Ppv, Qpv):
        ...
        J = torch.autograd.functional.jacobian(F_masked, z, create_graph=False, vectorize=False)
...
# ---- LIMITERS (bounds derived from equilibrium; toggles OFF by default) ----
        ...
        # === Newton policy (fast + stable) ===
        self.newton_itmax       = 6
        self.newton_tol         = 1e-5
        self.newton_backtracks  = 2   # cap Armijo to 1–2 trials
...
# ---------------- Interface required by ControlAffineSystem ----------------
    ...
    def state_limits(self) -> Tuple[Tensor, Tensor]:
        """Base expects (upper, lower) - keep on model device"""
...
# ---------------- AVR / Exciter ----------------
        ...
        F = torch.stack(f_list, dim=0)      # (B, n_dims)
        return F.unsqueeze(-1)              # (B, n_dims, 1)
    # ---------------- control matrix g(x) ----------------
    ...
    def _solve_kcl_newton(
        ...
        # Forward+Backward handled by the implicit layer (IFT)
        return _KCLNewtonImplicit.apply(self, delta, Eqp, Ppv, Qpv)
    ...
    # Add this method to IEEE39ControlAffineDAE class:
    def to(self, device):
        """Move all tensors to specified device"""
        device = torch.device(device) if isinstance(device, str) else device
        
        # List of tensor attributes to move (avoid computed properties)
        tensor_attrs = [
            'Y', 'G', 'B', 'PL_base', 'QL_base', 'Pg_target_total', 'Vset',
            'H_base', 'D_base', 'Xd_prime', 'Td0_prime',
            'Ka', 'Ta', 'R', 'Tg', 'Tt',
            ...
            'Pm_min', 'Pm_max',
            'max_dPvalve_up', 'max_dPvalve_dn',
            'Q_sg_max', 'Q_sg_min', 'k_qsuper',
            '_goal_point', '_u_eq',
            'Vref', 'Pref0',
            '_last_V', '_last_theta',
            '_V_goal', '_theta_goal',
        ...
        # Move each tensor attribute if it exists
