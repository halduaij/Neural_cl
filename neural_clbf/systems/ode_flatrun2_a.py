...
from neural_clbf.systems.IEEE39ControlAffineDAE_a import IEEE39ControlAffineDAE
...
def simulate_flat_run(
    ...
    Parameters:
    - pv_step: PV active power step [pu] to apply to chosen PV channels at t_step
    - t_step:  time to apply PV step [s]
    - pv_buses: indices (0..9) of PV channels to step; default: all
    - alter_events: list of AlterEvent(bus_idx, t, amount, tau). If None/empty, no load change.
    """
    ...
    # Initial state (equilibrium or perturbed)
    x = get_initial_state(sys, which=which_init, perturbation=perturbation)
    u = torch.zeros_like(sys.u_eq)  # IMPORTANT: don't inject drift at equilibrium

    pv_buses = list(pv_buses or range(sys.n_gen))  # default: step all PV-P controls
    ...
    if alter_mgr is not None:
        ...
        # preview (first event)
        b0 = alter_events[0].bus_idx
        print(f"[ALTER] reduced bus {b0}: sP_base={float(alter_mgr.sP_base[b0]):.6g}  sQ_base={float(alter_mgr.sQ_base[b0]):.6g}")
    ...
    V_hist = [V0.detach().cpu().numpy()]      # NEW: store bus |V|
    ...
    # First audit (should be near-zero if exact equilibrium)
    xn, Fn, dEfdn, domegan = audit_now(sys, x)
    xdot_norm.append(xn); kcl_norm.append(Fn); dEfd_norm.append(dEfdn); domega_norm.append(domegan)
    print(f"t={0.0:6.3f}  ||xdot||={xn:.3e}  ||KCL||={Fn:.3e}  ||dEfd||={dEfdn:.3e}  ||dω||={domegan:.3e}")
    if which_init == "equilibrium" and xn > 1e-4:
        print(f"WARNING: Starting from 'equilibrium' but ||xdot||={xn:.3e} > 1e-4")
    ...
    for k in range(N):
        ...
        # Optional discrete PV step (unchanged)
        if (pv_step != 0.0) and (t < t_step <= t_next):
            for i in pv_buses:
                u[0, i] += pv_step
            print(f"[{t_step:.3f}s] Applied PV P step of {pv_step:.4e} pu to buses {pv_buses}.")

        # --- NEW: monotone controller ---
        if use_monotone:
            u, V_now, th_now = monotone_linear_u(sys, x, droop_cfg)
        else:
            V_now, th_now = sys._solve_kcl_newton(
                sys._angle_reconstruct(sys._unpack_state(x[0])[0]),
                sys._unpack_state(x[0])[2],  # Eqp
                sys._unpack_state(x[0])[6],  # Ppv
                sys._unpack_state(x[0])[7],  # Qpv
            )

        # Integrate one step (ALTER loads updated inside RK stages)
        x = rk4_step(sys, x, u, dt, t_now=t, alter=alter_mgr)
        x = x.detach()  # forward-only

        t = t_next
        if (t - last_log_t) >= log_every - 1e-12 or k == N - 1:
            xn, Fn, dEfdn, domegan = audit_now(sys, x)
            times.append(t)
            xdot_norm.append(xn); kcl_norm.append(Fn); dEfd_norm.append(dEfdn); domega_norm.append(domegan)
            states_history.append(x.clone().detach().cpu().numpy())
            # NEW: store bus |V| at log instants
            V_hist.append(V_now.detach().cpu().numpy())
            print(f"t={t:6.3f}  ||xdot||={xn:.3e}  ||KCL||={Fn:.3e}  ||dEfd||={dEfdn:.3e}  ||dω||={domegan:.3e}")
            last_log_t = t

    # Restore base loads at end
    if alter_mgr is not None:
        alter_mgr.reset()
    ...
    states_history = np.array(states_history)[:, 0, :]  # remove batch dimension
    V_hist = np.array(V_hist)   # shape (len(times), n_buses)

    if plot:
        plot_results(sys, times, states_history, xdot_norm, kcl_norm, dEfd_norm, domega_norm,
                    pv_step, t_step, V_hist)   # pass V_hist
    if save_npz:
...
def main():
    ...
    ap.add_argument("--log-every", type=float, default=0.1, help="Logging interval [s]")
    ...
    # Build system (no PV by default; adjust nominal_params as needed)
    sys = IEEE39ControlAffineDAE(
       nominal_params = {
    "pv_ratio": [0.9, 0.9, 0.85, 0.9, 0.85, 0.85, 0.9, 0.9, 0.85, .3],  # Very high PV
    "T_pv": (0.01, 0.01)  # Fast PV dynamics (creates control interaction)
},
        dt=float(args.dt),
    )

    # ---- Gradient checks (no training, just autograd verification) ----
    if args.grad_check != "none":
        if args.grad_check == "u":
            ...
            # NOTE: use a smooth gate (tau > 0) if you later want gradients w.r.t. t0/tau.
        ...
        # Do not proceed to normal simulation
        return

    # Build ALTER events (if requested)
    events: List[AlterEvent] = []
        
    events = [
        AlterEvent(bus_idx=8, t=0.4, amount=1.20, tau=0.01),  
        AlterEvent(bus_idx=4, t=0.5, amount=1.25, tau=0.01),  # Hits during swing
        AlterEvent(bus_idx=6, t=0.6, amount=0.7, tau=0.01),   # Reversal during recovery
        
        AlterEvent(bus_idx=0, t=0.5, amount=1.15, tau=0.01),   # Area 1 up
        AlterEvent(bus_idx=9, t=0.5, amount=0.85, tau=0.01),   # Area 2 down
    ]
    
    if args.alter_amount not in (0.0, 1.0):
        if args.orig_buses:
            # Map original IEEE-39 bus numbers -> reduced (0..9) indices
            reduced_buses = []
            ...
            reduced_buses = args.alter_buses if args.alter_buses is not None else [9]  # default to orig bus 39
    ...
    simulate_flat_run(
        ...
        seed=int(args.seed),
        save_npz=args.save,
        alter_events=events,use_monotone=use_monotone, droop_cfg=cfg)
